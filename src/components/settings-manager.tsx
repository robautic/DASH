...
type Settings={workspace_name:string;workspace_slug:string;timezone:string;currency:string;locale:string;member_role:string}
type Member={user_id:string;email:string;display_name:string|null;role:'owner'|'admin'|'member'|'viewer';is_current_user:boolean}
type Invitation={invitation_id:string;email:string;role:string;status:string;created_at:string}
type Stage={pipeline_id:string;pipeline_name:string;stage_id:string;stage_name:string;stage_color:string|null;stage_position:number;is_won:boolean;is_lost:boolean;item_count:number}
