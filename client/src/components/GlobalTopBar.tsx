...
import { Search, Bell, Shield, User, ChevronDown, CheckCircle, Clock } from "lucide-react";
...
export function GlobalTopBar({ userRole, onRoleChange }: GlobalTopBarProps) {
  ...
  const mockSlaAlerts = [
    { id: "1", leadName: "Valeska Souza", timeOverdue: "18 min", attendant: "Ana Silva" },
    ...
    <header className="h-14 border-b border-[oklch(0.3_0.02_260/0.4)] bg-[oklch(0.12_0.02_260)] px-6 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="relative w-72 sm:w-96">
        ...
        <input
          ...
          placeholder="Pesquisa global de lead por nome, telefone ou ID..."
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Profile / Role Selector (Sprint 6) */}
        <div className="relative">
          <button
          ...
          >
            ...
            <span className="capitalize">{userRole === "admin" ? "Administrador" : userRole === "supervisor" ? "Supervisor" : "Atendente"}</span>
            <ChevronDown className="w-3.5 h-3.5 text-muted-foreground" />
          </button>

          {showRoleDropdown && (
            <div className="absolute right-0 top-10 w-48 glass-card p-1.5 space-y-1 z-50 border border-[oklch(0.3_0.02_260/0.4)] shadow-xl animate-in fade-in">
              <div className="px-2 py-1 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                Alternar Visão de Acesso
              </div>
              <button
                onClick={() => {
                  onRoleChange("admin");
              ...
              <button
                ...
                className={`w-full text-left px-2.5 py-1.5 rounded text-xs transition-colors flex items-center gap-2 ${
                  userRole === "supervisor" ? "bg-emerald-glow/15 text-emerald-glow font-bold" : "text-foreground hover:bg-white/5"
                }`}
              >
              ...
              <button
                ...
                className={`w-full text-left px-2.5 py-1.5 rounded text-xs transition-colors flex items-center gap-2 ${
                  userRole === "attendant" ? "bg-emerald-glow/15 text-emerald-glow font-bold" : "text-foreground hover:bg-white/5"
                }`}
              >
                <User className="w-3.5 h-3.5" />
                Atendente
